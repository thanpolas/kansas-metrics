...
var Metrics = module.exports = cip.extend(function() {
  ...
  this.kansas = configure.kansas;
  /** @type {Object} Internal options */
  this._options = configure._options;

  /** @type {logg} A logger instance for the fetch mixin */
  this.fetchLog = this.kansas.logger.getLogger('kansas-metrics.fetch');
...
Metrics.prototype.token = function(query) {
  if (Array.isArray(query)) {
    this.query.token = this.query.token.concat(query);
  } else {

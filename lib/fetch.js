...
Fetch.prototype._fetchKeys = Promise.method(function(queries) {
  return Promise.resolve(queries)
    ...
    .then(function(results) {
      this.canonicalKeys = __.uniq(__.flatten(results));
...
Fetch.prototype._fetchTokenItems = Promise.method(function(keys) {
  ...
  return Promise.resolve(tokenKeys)
    ...
    .then(function(results) {
      ...
      this.tokenItemIndex = {};
      results.forEach(function(tokenItem, index) {
        if (!tokenItem) {
          this.fetchLog.fine('_fetchTokenItems() :: Token item not found for:',
            tokenKeys[index]);
          return;
        }
 ...
 *
 * @param {Array.<string>} keys All keys to fetch usage records for.
 * @return {Array.<string>} Proper Kansas key paths.
 * @private
 */
